...
server <- function(input, output) {
  ...
  output$dynamic_selectbox <- renderUI({
    req(data_input())
    cols <- names(data_input())
  
    # 查找包含 "response" 的列
    response_col <- cols[grepl("response", cols, ignore.case = TRUE)]
    
    selectInput(
      "selectGroup",
      "Select response variables:",
      choices = cols,
      selected = if (length(response_col) > 0) response_col[1] else cols[1]
    )
  })  
  ...
  output$dynamic_selectbox_weight <- renderUI({
    ...
    # 查找包含 "weight" 的列
    weight_col <- cols[grepl("weight", cols, ignore.case = TRUE)]
    
    selectInput(
      "selectGroup_2",
      "Select weight variables if applicable:",
      choices = cols,
      selected = if (length(weight_col) > 0) weight_col[1] else cols[1]
    )
  })  
  ...
  # observe({
  #   req(is_running()) # 如果不是正在运行，停止执行
  #   invalidateLater(500) # 每0.5秒刷新一次
    
  #   # 简单的点点点逻辑
  #   current_text <- status_text()
  #   # 如果当前文本包含 "Running"，则处理点号 (防止覆盖掉 "Finished")
  #   if (grepl("Running", current_text)) {
  #     dots_count <- nchar(gsub("Running", "", current_text))
  #     new_dots <- if (dots_count >= 3) "" else paste0(rep(".", dots_count + 1), collapse = "")
  #     status_text(paste0("Running", new_dots))
  #   }
  # })
  ...
  #   # A. 锁定状态，显示 Modal
  #     is_running(TRUE)
  #     status_text("Running") # 初始状态
      
  #     showModal(modalDialog(
  #       title = "Running Analysis",
  #       # 这里不仅显示文本，最好加个加载条，显得更专业
  #       p(textOutput("running_status")), 
  #       footer = NULL,
  #       easyClose = FALSE
  #     ))
  ...
  #   # B. 【关键步骤】提取数据快照 (Snapshot)
  #   # 必须在主进程中把 reactive 数据变成普通数据
  #   df_snap <- data_input() 
  #   resp_var <- input$selectGroup
  #   w_var    <- input$selectGroup_2
  #   cat_vars <- input$checkGroup
  #   cont_vars <- input$checkGroup_2
  ...
  #     status_message(
  #       paste0("Analysis finished successfully! Time elapsed: ", elapsed, " seconds.")
  #     )
  ...
  #     # 5. (A) 失败时：在模态框中显示错误
  #     # (确保你的 status_message 足够长以显示错误)
  #     print(error) # 在 R 控制台打印完整错误
  #     status_message(
  ...
  #     # 6. 无论成功还是失败，总是在 2 秒后关闭模态框
  #     later::later(removeModal, 2)
...
observeEvent(input$show_table, {
  ...
  output$weighting_plot <- renderPlot({
    ...
    output$weighting_interactive <- renderPlotly({
      ...
      df_new <- df %>%
        mutate(
          difference = .data[[last_name]] - .data[[wname]],
          original_order = row_number()  # 添加原始排序列
        ) %>%
        arrange(.data[[wname]])

      p3 <- ggplot(df_new, aes(x = .data[[wname]], y = difference)) +
        geom_segment(aes(x = .data[[wname]], xend = .data[[wname]],
                        y = min(difference), yend = difference), color = "steelblue") +
        geom_point(aes(text = paste0(
          "Original Order: ", original_order, "<br>",
          wname, ": ", .data[[wname]], "<br>",
